...
class TrainingController extends Controller
{
    ...
    public function get_trainings( $id_training ){
        ...
        if ( $user_chapter_record->is_user_has_record(1,$id_training) == 'no') {
            ...
            if ($initiate != 'selesai') {
                return view('error')->with('error', $initiate);
            }
        }
        $count_finish_chapter = $user_chapter_record->check_finish_chapter( 1, $id_training);
        
        return view('intro_training')->with('training', $trainings)->with('module', $modul)->with('finish_chapter', $count_finish_chapter);
    ...
    public function get_material( $id_chapter ) {
        ...
        if ( $material_training['status'] == 'error') {
            return view('error')->with('error', $material_training)->with('module', $modul);
        }
        $chapter = new Chapter();
        $chapter = $chapter->get_chapter($material_training->id_chapter);
        if ($chapter['status'] == 'error') {
            return view('error')->with('error', $chapter)->with('module', $modul);
        }
        $chapter['material'] = $material_training;
        return view('material')->with('chapter', $chapter)->with('module', $modul);
    }
    ...
    public function finish_chapter( $id_chapter ) {
        ...
        if ($record['status'] == "error") {
            return view('error')->with('error', $record)->with('module', $modul);
        }
    ...
    public function get_test ( $id_chapter ) {
        ...
        if ($test['status'] == 'error') {
            return view('error')->with('error', $test)->with('module', $modul);
        }
        $chapter = new Chapter();
        ...
        if ($user_test_record['status'] == 'error') {
            return view('error')->with('error', $user_test_record)->with('module', $modul);
        }
        ...
        if ($record_chapter['status'] == "error") {
            return view('error')->with('error', $record_chapter)->with('module', $modul);
        }

        $chapter['test'] = $test;

        return view('online_test')->with('chapter', $chapter)->with('test',$test)->with('module',$modul);
    ...
    public function review_test( $id_chapter) {
        ...
        return view('online_test_review')->with('chapter', $chapter)->with('record', $user_record)->with('module', $modul);
    }
